...
const Post = ({ data, loggedInUser, isGroup, isMember, isVisible }) => {
  ...
  async function getOwnerInfo() {
    const user = await getUsers_service(data.userId);
  ...
  return (
    <div className="groups-post-wrapper">
      ...
      <div className="groups-post-wrapper__edit-modal">
        <PopUp
          header={`${popUpType} post`}
          onClose={
            popUpType === "Edit"
              ? () => setEditMode(false)
              : () => showConfirmation(false)
          }
          shown={popUpType === "Edit" ? editMode : showDeletePopup}
          centered="true"
          body={
            popUpType === "Edit"
              ? postTextArea
        ...
        >
          <Button
            className={popUpType === "Edit" ? "submit-btn" : "warning-btn"}
            text={popUpType === "Edit" ? "Save" : "Delete"}
            size={"medium"}
            onSubmitHandler={
              popUpType === "Edit" ? () => editPost() : () => deletePost()
            }
          />
          <Button
